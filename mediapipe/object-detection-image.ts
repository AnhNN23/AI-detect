...
const ObjectDetectionImage = (() => {
  ...
  let runningMode: RunningMode = RUNNING_MODE_IMAGE;
  let delegate: DelegateMode = "CPU";
  ...
  const initModel = async (vision: any): Promise<ModelLoadResult> => {
    ...
    try {
      if (vision) {
        ...
        objectDetector = await ObjectDetector.createFromOptions(vision, config);
  ...
  const detectObject = async (
    ...
    if (objectDetector) {
      try {
        ...
        console.log("detectObjectImage:", image);
        const detection: ObjectDetectorResult = await objectDetector.detect(
          image
        );
        ...
        detection.detections.forEach((detected) => {
          if (
            ...
            (detected.boundingBox.width === 0 ||
              detected.boundingBox.height === 0)
          ) {
            console.error(
              "Bounding box has zero dimension:",
              detected.boundingBox
            );
          }
        });
        ...
        if (error instanceof Error) {
          ...
          console.error("Unknown error during object detection:", error);
        }
      }
    }
    return null;
      ...
      detections.forEach((detected: Detection) => {
        if (detected.boundingBox) {
          ...
          const label = Drawing3d.createLabel(
            ...
            category.score,
            category.categoryName === "person" ? "#FF0F0F" : "#00B612",
            width,
            height,
            mirrored,
            box
